...
class PageChildPages extends DataSource
{
    public $db_link;
    public $db_link_flag;
    public $pk = ['id'];
    ...
    public function query($parameters, $options = [])
    {
        ...
        if (empty($parameters['only_one_parent'])) {
            $query = Builder::quick()->withRecursive('temp_page_1000', ['module_id', 'id', 'parent_id', 'order2'], function (& $query) use ($parameters) {
                $query = Pages::queryBuilderStatic(['skip_acl' => true, 'alias' => 'inner_a'])->select();
                $query->columns([
                ...
                $query->union('UNION ALL', function (& $query2) {
                    $query2 = Pages::queryBuilderStatic(['skip_acl' => true, 'alias' => 'inner_b'])->select();
                    $query2->columns([
            ...
            $query = Pages::queryBuilderStatic(['alias' => 'inner_a'])->select();
            $query->columns([
                'module_id' => 'inner_a.dn_repopage_module_id',
                'id' => 'inner_a.dn_repopage_id',
                'parent_id' => 'inner_a.dn_repopage_parent_repopage_id',
