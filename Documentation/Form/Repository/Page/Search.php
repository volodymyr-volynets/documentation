...
use Numbers\Documentation\Documentation\Model\Repository\Version\Page\Translations;
use Numbers\Documentation\Documentation\Model\Repository\Version\Pages;
use Object\Form\Wrapper\List2;

class Search extends List2
{
    ...
    public function overrides(& $form)
    {
        ...
        // auto submit
        if (!empty($form->values['full_text_search'])) {
            \Layout::onLoad(<<<JST
                if ($('.numbers_frontend_form_list_pagination_container:visible', $('#form_dn_page_repository_search_element___submit_button').closest('form')).length == 0) {
                    $('#form_dn_page_repository_search_element___submit_button').click();
                }
JST);
        }
    }
    ...
    public function listQuery(& $form)
    {
        ...
        // primary model
        $form->query = Pages::queryBuilderStatic(['alias' => 'a']);
        ...
        if ($repository['dn_repository_default_language_code'] == $form->values['dn_repository_language_code']) {
            ...
            $form->query->columns([
                ...
                'dn_repopage_rank' => 'COALESCE(b.fragments_ts_rank, 0) + ' . $fts_titles['rank_simple'],
            ...
            $form->query->join('LEFT', function (& $query) use ($form) {
                ...
                $query->columns([
                    ...
                    'fragments_ts_rank' => "SUM(COALESCE({$fts_fragments['rank_simple']}, 0))",
            ...
            $form->query->join('INNER', new Translations(), 'e', 'ON', [
    ...
    public function renderPageId(& $form, & $options, & $value, & $neighbouring_values)
    {
        ...
        $href = \Request::buildURL(\Application::get('mvc.controller') . '/_Edit/' . $hash . '/' . $filename, [], null, 'page_title');
    ...
    {
        if (isset($value)) {
            $temp = Parser::extractSentances($form->values['full_text_search'], $value, 5, 100);
