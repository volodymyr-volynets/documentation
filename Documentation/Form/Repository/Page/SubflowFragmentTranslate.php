...
class SubflowFragmentTranslate extends Base
{
    ...
    public function refresh(& $form)
    {
        ...
        if (!empty($form->misc_settings['__form_onchange_field_values_key'][0])) {
            if ($form->misc_settings['__form_onchange_field_values_key'][0] == 'dn_repofragtransl_language_code') {
                $translation = Translations::getStatic([
                    'where' => [
    ...
    public function validate(& $form)
    {
        ...
        if (empty($form->values['dn_repofragtransl_name']) && empty($form->values['dn_repofragtransl_body'])) {
            $form->error(DANGER, Messages::OPTIONALLY_REQUIRED_FIELD, 'dn_repofragtransl_name');
            $form->error(DANGER, Messages::OPTIONALLY_REQUIRED_FIELD, 'dn_repofragtransl_body');
        }
        // keywords
