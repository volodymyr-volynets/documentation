...
class SubflowFragmentFileNew extends Base
{
    ...
    public function refresh(& $form)
    {
        if ($form->values['dn_repopgfragm_type_code'] == 'IMAGE') {
            $form->element('top', 'dn_repopgfragm_file_1_new', 'dn_repopgfragm_file_1_new', [
                ...
                'description' => implode(', ', Validate::$validation_extensions['images']),
            ]);
        } else {
            $form->element('top', 'dn_repopgfragm_file_1_new', 'dn_repopgfragm_file_1_new', [
                'description' => implode(', ', array_merge(Validate::$validation_extensions['images'], Validate::$validation_extensions['audio'], Validate::$validation_extensions['documents'])),
            ]);
        }
        // preset order
        if (empty($form->values['dn_repopgfragm_id']) && empty($form->values['dn_repopgfragm_order'])) {
            $last_fragment = Fragments::getStatic([
                'where' => [
