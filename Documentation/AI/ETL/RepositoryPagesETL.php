...
class RepositoryPagesETL
{
    public static function process()
    {
        ...
        return \Db::etl(function ($options) {
            $query = Pages::queryBuilderStatic(['alias' => 'a'])
                ->select()
                ->columns([
                    'a_id' => "concat_ws('::', a.dn_repopage_tenant_id, a.dn_repopage_module_id, a.dn_repopage_id)",
                    'a.*',
                    'r.*',
                    'm.*',
                    'd.*'
                ...
                ->join('INNER', new Modules(), 'm', 'ON', [
                    ['AND', ['m.tm_module_id', '=', 'a.dn_repopage_module_id', true], false],
                ])
                ->join('INNER', new Repositories(), 'r', 'ON', [
                    ['AND', ['r.dn_repository_module_id', '=', 'a.dn_repopage_module_id', true], false],
                    ['AND', ['r.dn_repository_id', '=', 'a.dn_repopage_repository_id', true], false],
                ])
                ->join('LEFT', new Records(), 'b', 'ON', [
                    ['AND', ['b.tm_batchrecord_tm_batchtype_code', '=', 'DN_EMBEDDINGS', false], false],
                    ['AND', ['b.tm_batchrecord_module_id', '=', 'a.dn_repopage_module_id', true], false],
                    ['AND', ['b.tm_batchrecord_field_code', '=', 'dn_repopage_id', false], false],
                    ['AND', ['b.tm_batchrecord_no_data_model_role_code', '=', 'primary', false], false],
                    ['AND', ['b.tm_batchrecord_field_value_id', '=', "a.dn_repopage_id", true], false]
                ])
                ->join('LEFT', new Records(), 'c', 'ON', [
                    ['AND', ['c.tm_batchrecord_tm_batchtype_code', '=', 'DN_EMBEDDINGS', false], false],
                    ['AND', ['c.tm_batchrecord_module_id', '=', 'a.dn_repopage_module_id', true], false],
                ...
                ->join('LEFT', new Embeddings(), 'd', 'ON', [
                    ...
                    ['AND', ['d.dn_embedding_module_id', '=', 'a.dn_repopage_module_id', true], false],
            ...
            $content = RepositoryPages2Embedding::transform($row, $options)['content'];
            $transform = [
                'a_id' => $row['a_id'],
                // page
            ...
            'dn_embedding_ai_ragtype_code' => 'DN::PAGES',
        ]);
    }
}
