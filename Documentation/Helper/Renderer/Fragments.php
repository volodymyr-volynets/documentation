...
use Helper\Parser;
use Numbers\Documentation\Documentation\DataSource\Repository\FragmentLanguages;
use Numbers\Documentation\Documentation\Helper\Pages;
use Numbers\Internalization\Internalization\Helper\Languages;
...
class Fragments
{
    ...
    public static function renderForm(& $form, & $options, & $value, & $neighbouring_values)
    {
        ...
        foreach ($form->values['\Numbers\Documentation\Documentation\Model\Repository\Version\Page\Fragments'] as $k0 => $v0) {
            ...
            } elseif (in_array($v0['dn_repopgfragm_type_code'], ['FILE', 'IMAGE'])) {
                $menu['options']['edit']['options'] = [
            ...
            if (!empty($v0['\Numbers\Documentation\Documentation\Model\Repository\Version\Page\Fragment\Translations'])) {
                ...
                $language = Languages::renderOneLanguage($temp['dn_repofragtransl_language_code']);
            } else {
                $language = Languages::renderOneLanguage($v0['dn_repopgfragm_language_code']);
            }
            $languages2 = '<div class="form_dn_page_repository_page_view_form_language">';
            $languages2 .= i18n(null, 'Language') . ': ' . $language . ' ';
            $languages = FragmentLanguages::getStatic([
                'where' => [
            ...
            foreach ($languages as $k => $v) {
                $temp[] = Languages::renderOneLanguage($k);
            }
    ...
    public static function renderFormTEXT($value)
    {
        ...
        $result .= \HTML::div(['value' => $body, 'class' => 'form_dn_page_repository_page_view_form_fragment_holder_text']);
        return $result;
    }
    ...
    public static function renderFormFILE($value)
    {
        ...
        if (!empty($translation)) {
            $name = $translation['dn_repofragtransl_name'] ?? $value['dn_repopgfragm_name'];
            $body = Files::generateURLS($translation, 'dn_repofragtransl_file_', 10);
        } else {
            $name = $value['dn_repopgfragm_name'];
            $body = Files::generateURLS($value, 'dn_repopgfragm_file_', 10);
        }
    ...
    public static function renderFormIMAGE($value)
    {
        ...
        if (!empty($translation)) {
            $name = $translation['dn_repofragtransl_name'] ?? $value['dn_repopgfragm_name'];
            $temp_body = Files::generateOnlyURLS($translation, 'dn_repofragtransl_file_', 10);
        } else {
            $name = $value['dn_repopgfragm_name'];
            $temp_body = Files::generateOnlyURLS($value, 'dn_repopgfragm_file_', 10);
        }
    ...
    {
        $matches = Parser::match($body, '[href[', ']]', ['all' => true]);
        $translation = current($value['\Numbers\Documentation\Documentation\Model\Repository\Version\Page\Fragment\Translations']);
        if (!empty($matches)) {
            foreach ($matches as $k => $v) {
                $page = Pages::fetchOnePage($value['dn_repopgfragm_module_id'], $value['dn_repopgfragm_repository_id'], $value['dn_repopgfragm_version_id'], $value['dn_repopgfragm_language_code'], $v);
                $hash = \Request::hash([
